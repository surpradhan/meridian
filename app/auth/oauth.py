...
⚠️  Multi-process / multi-worker WARNING
---------------------------------------
``_STATE_STORE`` is an in-process dict.  In a single-worker deployment (the
default ``uvicorn`` invocation) this works correctly.  If you run multiple
worker processes (e.g. ``gunicorn -w 4`` or Kubernetes replicas), the state
token generated by worker A will NOT be found by worker B on the OAuth
callback, causing every login to fail with "Invalid or expired state".

To support multi-process deployments, replace ``_STATE_STORE`` and its helper
functions with a Redis-backed implementation (e.g. via ``redis-py`` with a
short TTL).  Set ``REDIS_URL`` in your environment and wire it here before
going to production with multiple workers.
"""
...
class OAuthManager:
    ...
    async def handle_callback(
        ...
        name = userinfo.get("name") or userinfo.get("given_name") or (email.split("@")[0] if email and "@" in email else "")
